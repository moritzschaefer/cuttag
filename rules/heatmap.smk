...
rule filter_peaks_summit:
    input:
        'peaks/{sample}_top{percent}percent.bed'
    output:
        'peaks/{sample}_top{percent}percent_summit.bed'
    conda:
        '../env.yaml'
    shell: '''
...
rule generate_peak_matrix:
    input:
        ...
        peak_summit_bed="peaks/{sample}_top{percent}percent_summit.bed"
    output:
        'heatmaps/{sample}_top{percent}percent_peaks.mat.gz'
    params:
        cores=20
    conda:
        '../env.yaml'
...
rule plot_peak_heatmap:
    input:
        'heatmaps/{sample}_top{percent}percent_peaks.mat.gz'
    output:
        'heatmaps/{sample}_top{percent}percent_peaks.png'
    conda:
        '../env.yaml'
    shell: '''
